<template>
  <div class="new-header">
        ...
        <div class="sjContainer">
          <div class="sj">
              <div class="sj-box clearfix">
                <a class="time" href="javascript:void(0)" style="color:#fff;margin-left:8px"><span id="EST_reciprocal">{{time}}</span></a>
                <ul class="clearfix">
              ...
              <div class="login-before">
                  <div class="form">
                    ...
                    <div class="loginAfter" v-if="loggedIn == true">
                      ...
                      <div><span @click="$goUserCen('recharge',0)" >在线存款</span></div>
                      ...
                      <!-- <div @click="getUrl">
                        <span><img src="/static/737qp/img/icon_ser.png" alt=""></span>
                        <span>在线客服</span>
                      </div> -->
                    </div>
                  </div>
    ...
    <div class="nav">
      <div class="navBox">
        <div class="navItems">
          <span @mouseover="getgameData(item)" :class="['navItem',item.text]" v-for="(item,i) in classifyList.slice(0,4)" :key="i" @click.stop="$goPath('one',item)">
            <a href="javascript:0" @click.stop="$goPath('one',item)">{{item.name}}</a>
          </span>
        </div>
        <div class="logo" @click="$router.push('/')"><img src="/static/737qp/logo/logo.png" alt=""></div>
        <div class="navItems">
          <span @mouseover="getgameData(item)" :class="['navItem',item.text]" v-for="(item,i) in classifyList.slice(4,11)" :key="i" @click.stop="$goPath('one',item)">
            <a href="javascript:0" @click.stop="$goPath('one',item)">{{item.name}}</a>
          </span>
        </div>
        <div :class="['slideShow',showClass?'newClass':'']" @mouseleave="clearData" >
          <div class="smallWidth" v-show="gameArr.length < 10">
            <ul>
              <li :class="gamename" v-for="(item,index) in gameArr" :key="index">
                <span><img :src="'/static/737qp/img/new_games/new_icon/'+item.id+'.png'" ></span>
          ...
          <div class="slideContent" v-show="gameArr.length > 9">
            ...
            <swiper :options="swiperOption" ref="mySwiper" >
                <swiper-slide v-for="(item,index) in gameArr" :key="index">
                    <div :class="['slide_list',gamename]" >
                        <span v-if="lotteryImg"><img width="70px" :src="'/static/737qp/img/new_games/lottery/'+item.id+'.svg'" ></span>
                        <span v-else><img :src="'/static/737qp/img/new_games/new_icon/'+item.id+'.png'" ></span>
                        <span>{{item.name}}</span>
                        <span @click="gogames(item)">进入游戏</span>
                    </div>
                </swiper-slide>
    ...
    return {
      ...
      classifyList: [
        ...
        },
        {
          ...
          list: [],
          type: "chess",
          pImg: "/static/737qp/img//hot11.gif",
          hot: true,
          arr:[],
        },
        {
      ...
      showmodle: false,
      codeImg: "/static/737qp/img/lo.png",
      left: 5,
      top: -193,
      showReload: false,
      gameArr:[],
  ...
  methods: {
    ...
    gogames(item){
      ...
      switch (this.gamename) {
        case "AG_GAME":
          this.$router.push('/home/slot?navid=9&id='+item.id)
          break;
        case "KY_CHESS":
          this.$router.push('/home/chess?navid=9&id='+item.id)
          break;
        case "fish_game":
    ...
    getNativeDatas() {
        if (localStorage.chess_game_new) {
          let a = JSON.parse(localStorage.gameSortV4_chess)
          let b = JSON.parse(localStorage.gameSortV4_slot)
          let c = JSON.parse(localStorage.gameSortV4_fish)
          let d = JSON.parse(localStorage.gameSortV4_live)
          let e = JSON.parse(localStorage.gameSortV4_sport)
          this.classifyList[1].arr = a[0].list.slice(-9);
          this.classifyList[2].arr = b[1].list.slice(-10);
          this.classifyList[3].arr = c[0].list.slice(-14);
          this.classifyList[1].arr = this.classifyList[1].arr.filter(ele => ele.name !='全部' )
          this.classifyList[2].arr = this.classifyList[2].arr.filter(ele => ele.name !='全部' )
           this.classifyList[3].arr = this.classifyList[3].arr.filter(ele => ele.name !='全部' )
          this.classifyList[4].arr = this.cpArr;
          this.classifyList[5].arr = d.games;
          this.classifyList[6].arr = e.games;
        }
    },
    getgameData(item){
      console.log(item);
    ...
    goHeader(typeNum) {
      if (typeNum == 0) {
        ...
        window.open("/static/737qp/html/download/index.html");
      } else if(typeNum == 3){
        window.open("http://737.cc");
      }
    },
    getUrl() {
      let service = JSON.parse(localStorage.config).service;
      ...
      .navBox{
        ...
        .navItems{
          ...
          span{
            ...
            height: 85px;
            line-height: 85px;
            background: url(/static/737qp/img/arro.png) 100% no-repeat;
