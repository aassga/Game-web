  ...
  <div class="header index-header">
    ...
    <div class="header-top">
      <div class="w1000">
        <div class="toplink">
          <div class="lisence">
            <img src="/static/js85/img/pz.png" />
          </div>
          <a class="time">
            美东时间：
            <span id="mdtime" data-nowtime="2019/03/29 02:34:04">{{
              time
            }}</span>
          </a>

          <div class="commonFunc">
            <!-- <a href="javascript:;" class="colors-change detection" @click="goHeader('agent')">代理加盟</a>| -->
            <a
              href="javascript:;"
              class="colors-change detection"
              @click="goHeader(0)"
              >线路检测</a
            >|
            <a
              href="javascript:;"
              class="colors-change jiebei"
              @click="goHeader(1)"
              >免息借呗</a
            >|
            <a
              href="javascript:;"
              class="colors-change guanjia"
              @click="goHeader(2)"
              >金管家</a
            >|
            <a
              href="javascript:;"
              class="colors-change download"
              @click="goHeader(4)"
              >下载APP</a
            >|
            <a
              href="javascript:;"
              class="colors-change suggestion"
              @click="goUserCen('message', 1)"
              >投诉建议</a
            >
          </div>
        </div>
          ...
          <ul id="head-container">
            ...
            >
              ...
              >
                ...
                <span
                  class="ch"
                  :class="[item.hot == true ? 'blingColor' : '']"
                  >{{ item.name }}</span
                >
                <img v-if="item.hot" src="/static/js85/img/hot.gif" alt="hot" />
              </a>
              <div class="subnav" v-if="item.list && item.list.length > 0">
                ...
                <div class="con">
                  ...
                  >
                    {{ v.name }}
                    <img
                      src="/static/js85/img/hot.gif"
                      style="position: absolute; z-index: 99; right: 5px"
                      v-if="
                        [
                          173, 11421, 11729, 10022, 10018, 10021, 11320, 243,
                          10613, 10695,
                        ].includes(v.id)
                      "
                    />
    ...
    <div class="lgobox" :style="{ height: carheight + 'px' }" v-if="logShow">
      <div class="index-bannercon w1000" v-if="isShow && !userinfo">
        <div name="LoginForm" id="loginForms">
          <div class="index-login" :class="[code_show ? 'withCode' : 'noCode']">
            ...
            <a
              class="register_btn"
              :class="[code_show ? 'show_code' : 'no_code']"
              href="javascript:void(0)"
              @click="goKaihu"
            ></a>
          </div>
    ...
    <div class="header-login" v-if="$store.state.mainState.loginOrout">
      <div class="w1000">
        ...
        <div class="right">
          <span>
            <a href="javascript:void(0)" @click="$bindPhoneOrbank()">线上存款</a
            >&nbsp; | &nbsp;
            <a href="javascript:void(0)" @click="$goUserCen('withdraw', 0)"
              >线上取款</a
            >&nbsp; | &nbsp;
            <a href="javascript:void(0)" @click="$goUserCen('personage', 2)"
              >下注记录</a
            >&nbsp; | &nbsp;
            <a href="javascript:void(0)" @click="$goUserCen('discounts', 0)"
              >实时返水</a
            >&nbsp; | &nbsp;
            <a
              href="javascript:void(0)"
    ...
    <div
      ...
      id="xheader-login"
      style="display: block"
      v-if="!isShow && !$store.state.mainState.loginOrout"
    >
      <form
        name="LoginForm"
        id="loginForm"
      ...
      >
        <div class="header-logincon">
          ...
          <!-- 验证码 -->
          <div class="verify_code_box" v-if="code_show && !isShowTnCode">
            <input
              type="text"
              class="code"
              id="rmNum"
...
import freshButton from "../../public/home/freshButton.vue";
export default {
  mixins: [data1],
  data() {
    return {
      ...
      headerImg: "/static/js85/img/tiger.jpg",
      time: "",
      lantern: "",
      isShow: true,
      showLittlePic: true,
  ...
  methods: {
    ...
    ctrNavHide(event) {
      $(event.target).children("div").hide();
    },
    goKaihu() {
    ...
    goHeader(typeNum) {
      if (typeNum == 0) {
        // 线路检测
        window.open("http://422.com/", "_blank");
      } else if (typeNum == 1) {
        // 借呗
        // window.open('/static/js85/html/active/jiebei/index.html');
        ...
        window.open("/static/js85/html/download/index.html");
      } else if (typeNum == "agent") {
        // 代理加盟
...
.index-header {
  ...
  .header-float {
    ...
    .navbg {
      ...
      .nav {
        ...
        .logo {
          background: url(/static/js85/img/home_logo.png) no-repeat center;
          ...
          li {
            ...
            a {
              ...
              .ico {
                ...
                margin-bottom: 10px;
                &.home {
                  background: url(/static/js85/img/homeico.png) no-repeat center
                    center;
                }
                &.elec {
                  background: url(/static/js85/img/elecico.png) no-repeat center
                    center;
                }
                &.chess {
                  background: url(/static/js85/img/chessico.png) no-repeat
                    center center;
                }
                &.casino {
                  background: url(/static/js85/img/casinoico.png) no-repeat
                    center center;
                }
                &.lottery {
                  background: url(/static/js85/img/lotteryico.png) no-repeat
                    center center;
                }
                &.fishing {
                  background: url(/static/js85/img/fishingico.png) no-repeat
                    center center;
                }
                &.esport {
                  background: url(/static/js85/img/esportico.png) no-repeat
                    center center;
                }
                &.promo {
                  background: url(/static/js85/img/promoico.png) no-repeat
                    center center;
                }
                &.service {
                  background: url(/static/js85/img/serviceico.png) no-repeat
                    center center;
                }
              }
              img {
    ...
    .index-bannercon {
      #loginForms {
        ...
        .index-login {
          ...
          padding-top: 28px;
          &.withCode {
            background: url(/static/js85/img/login/with_code.png) no-repeat;
            .subbtn {
              bottom: 10px;
            }
          }
          &.noCode {
            background: url(/static/js85/img/login/no_code.png) no-repeat;
            .subbtn {
              bottom: 64px;
            }
          }
          ...
          .title {
            ...
            height: 24px;
            margin-bottom: 12px;
            background: url(/static/js85/img/login/title.png) center center
              no-repeat;
          }
          p {
            width: 82%;
            ...
            &.username_box {
              margin-bottom: 10px;
              background: url(/static/js85/img/login/user_input.png) no-repeat
                center center;
            }
            &.pwd_box {
              background: url(/static/js85/img/login/pwd_input.png) no-repeat
                center center;
            }
            &.code_box0 {
              height: 48px;
          ...
          .button {
            ...
            a {
              ...
              &.login_btn {
                text-decoration: none;
                background: url(/static/js85/img/login/login_btn.png) no-repeat
                  center center;
                background-size: contain;
              ...
              &.forget {
                background: url(/static/js85/img/login/forget_pwd.png) no-repeat
                  center center;
                background-size: contain;
              }
            }
          ...
          .register_btn {
            display: block;
            background: url(/static/js85/img/login/register_btn.png) no-repeat
              center center;
            width: 83%;
            height: 42px;
            &.show_code {
  ...
  .header-login {
    ...
    #loginForm {
      ...
      height: 100%;
      .header-logincon {
        background: url(/static/js85/img/headerlogin.png) left center no-repeat;
        height: 100%;
        width: 1200px;
        margin: 0 auto;
        display: flex;
        ...
        .forget {
          background: url(/static/js85/img/forget.png) no-repeat;
          width: 60px;
          height: 32px;
          display: block;
          float: left;
        ...
        .subbtn {
          background: url(/static/js85/img/subbtn.png) no-repeat;
          width: 130px;
          height: 34px;
          border: none;
          padding: 0;
        ...
        .regbtn {
          background: url(/static/js85/img/regbtn.png) top center no-repeat;
          width: 130px;
          height: 34px;
          display: block;
          float: left;
        ...
        span {
          ...
          &.refresh_balance {
            .refresh {
              ...
              width: 22px;
              height: 20px;
              background: url("/static/js85/img/refresh.png") center center
                no-repeat;
              // transform: scale(1.2);
              margin-top: 17px;
            }
            .newfresh {
              background: url("/static/js85/img/newfresh.png") no-repeat center;
            }
            .fres {
              -webkit-animation: spin 1s linear;
              animation: spin 1s linear;
        ...
        span {
          a {
            ...
            &.logOut {
              ...
              text-align: center;
              color: #000;
              background: url(/static/js85/img/log_out.png) center center
                no-repeat;
              background-size: contain;
            }
          }
...
.header .nav .subnav .jt {
  background: url(/static/js85/img/menujt.png) no-repeat;
  height: 5px;
  width: 9px;
  display: block;
  margin: 0 auto;
