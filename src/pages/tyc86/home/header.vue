    ...
    <div class="header-top" :class="{ night: colorbool }">
          ...
          <div class="sj">
            <div class="sj-box clearfix">
              <a
                ...
                style="color: #d060d2; margin-left: 8px"
                ><span id="EST_reciprocal">{{ time }}</span></a
              >
              <ul class="clearfix">
                <li>
                  <a
      ...
      <div class="nav">
        <div class="navBox">
          <div class="navItems">
            <span
              @mouseover="getgameData(item)"
              :class="['navItem', item.text]"
              v-for="(item, i) in classifyList.slice(0, 4)"
              :key="i"
          ...
          <div class="logo" @click="$router.push('/')">
            <img src="/static/tyc86/logo/logo.png" alt="" />
          </div>
          <div class="navItems">
            <span
              @mouseover="getgameData(item)"
              :class="['navItem', item.text]"
              v-for="(item, i) in classifyList.slice(4, 11)"
              :key="i"
              @click="$goPath('one', item)"
            >
              <a href="javascript:void(0);" @click="$goPath('one', item)">{{
          ...
          <div
            :class="['slideShow', showClass ? 'newClass' : '']"
          ...
          >
            <div class="smallWidth" v-if="gameArr.length < 10">
              <ul>
                ...
                >
                  <span
                    v-if="gamename == 'KY_CHESS' || gamename == 'AG_GAME'"
                    class="qw"
                    ><img
                      :src="
                        '/static/tyc86/img/new_games/new_icon/' +
                        item.gcid +
                        '.png'
                      "
                  /></span>
              ...
              <swiper :options="swiperOption" ref="mySwiper">
                <swiper-slide v-for="(item, index) in gameArr" :key="index">
                  <div :class="['slide_list', gamename]">
                    <span v-if="lotteryImg"
                      ><img
                        width="70px"
                        :src="
                          '/static/tyc86/img/new_games/lottery/' +
                          item.id +
                          '.svg'
                        "
                    /></span>
    ...
    return {
      ...
      classifyList: [
        ...
        },
        {
          ...
          list: [],
          type: "chess",
          pImg: "/static/tyc86/img//hot11.gif",
          hot: true,
          arr: [],
        },
        {
      ...
      showmodle: false,
      codeImg: "/static/tyc86/img/lo.png",
      left: 5,
      top: -193,
      showReload: false,
      gameArr: [],
  ...
  methods: {
    ...
    getNativeDatas() {
      if (localStorage.gameSortV4_chess) {
        this.classifyList[2].arr = JSON.parse(localStorage.gameSortV4_chess)[1]
          .list.filter((item) => item.name != "全部")
          .slice(-9);
      }
      ...
      if (localStorage.gameSortV4_slot) {
        this.classifyList[3].arr = JSON.parse(localStorage.gameSortV4_slot)[1]
          .list.filter((item) => item.name != "全部")
          .slice(-10);
      }
    ...
    goHeader(typeNum) {
      if (typeNum == 0) {
        window.location.href = "/static/tyc86/html/download/index.html";
      } else if (typeNum == 1) {
        // 借呗
        if (localStorage.token) {
        ...
        window.open("/static/tyc86/html/download/index.html");
      } else if (typeNum == 3) {
        window.open("http://285.cc");
      } else if (typeNum == 5) {
        window.location.href = "http://653.com";
      }
    },
    getUrl() {
      let service = JSON.parse(localStorage.config).service;
      ...
      .navBox {
        ...
        .navItems {
          ...
          span {
            ...
            height: 85px;
            line-height: 85px;
            background: url(/static/tyc86/img/arro.png) 100% no-repeat;
