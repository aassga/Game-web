...
export default {
  ...
  methods: {
    ...
    setGameMaker(item) {
      this.iptVal='';
      let scrollTop = document.documentElement.scrollTop||document.body.scrollTop;
      if (this.maker_active !== item.id) {
				this.$router.push({ path: '/home/qipai', query: { navid: 9, id:item.id } })
    ...
    cancelCollect(item,index) {
      ...
      if (!localStorage.token || !localStorage.userinfo) {
		...
		//获取棋牌全部参数 是否在session里 不在则获取
    async getGameList() {
      let chessGame = sessionStorage.getItem("chessgame");
      ...
      if (this.listData.label.length > 0) {
        ...
        for(var i=0;i<this.listData.label.length;i++){//热门游戏列表
          if(this.listData.label[i].name=='热门游戏'){
        ...
        .game_type {
          ...
          li {
            ...
            div {
              ...
              .hot_game {
                background: url("/static/public/image/casino/youyi/qb.png")
                  center top no-repeat;
							}
							.vg{
								background: url("/static/public/image/casino/vg-vnst.png")
									center top no-repeat;
							}
							.ky{
								background: url("/static/public/image/casino/ky-vnst.png")
									center top no-repeat;
							}
							.ly{
								background: url("/static/public/image/casino/ly-vnst.png")
									center top no-repeat;
							}
