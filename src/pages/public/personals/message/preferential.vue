    ...
    <div class="content clearfloat" v-if="messageData.length">
      <div class="left fl">
        ...
        <div class="row clearfloat" v-for="(item,i) in messageData" :key="i" @click="toggle(i,item)">
          ...
          <span class="system">
            系统
              <span class="systemmessage" v-if="item.status=='no'">1</span>
          </span>
        ...
        <div class="page">
          <Page :current="i" :total="total" size="small" :page-size="5" @on-change="hanlderPage" :styles="{fontSize:'14px'}"></Page>
        </div>
      </div>
      <div class="right fl">
        <div class="title">
        ...
        <div class="times">
          {{this.$moment.unix(messageDetail.updated_at-0).format('YYYY-MM-DD HH:mm:ss')}}
        </div>
        <div class="content" v-html="messageDetail.content">
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    getmessage() {
      ...
      this.$http
        .post(`${this.$HOST_NAME}/member/message?page=` + this.i,{
            limit: 5
        })
        .then(res => {
          if (res.code == 200) {
            ...
            if(res.data.data.length>0){
              res.data.data && res.data.data.forEach(v => {
                v.updated_at = v.send_time;
                v.day = getMyDay(this.$moment.unix(v.send_time - 0).format("d"));
                v.send_time = this.$moment.unix(v.send_time - 0).format("MM-DD");
              });
              this.messageData = res.data.data;
              this.total = res.data.total;
              this.messageDetail = res.data.data[0];
              if (this.messageDetail.status == "no") {
                this.updateSyMsg(this.messageDetail.id);
              }
              this.messageData[0].status = "yes";
            } 
          }
        });
    },
    ...
    async toggle(i, item) {
      ...
      if (item.status == "no") {
        this.messageData[i].status = "yes";
        this.updateSyMsg(item.id);
      }
    },
    hanlderPage(i) {
      this.i = i;
