  ...
  <div class="apply_open">
    ...
    <div class="header">
      <ul>
        ...
        <li class="aisle" v-if="personal_commission_mode!='mode_b'">
          <span :class="{spanActive:spanActive ==1}" @click="toggle(1)">{{site_model=="invite_code"?"创建邀请码":"创建邀请链接"}}</span>
        </li>
        <li class="aisle">
          <span :class="{spanActive:spanActive ==2}" @click="toggle(2)">{{site_model=="invite_code"?"管理邀请码":"管理邀请链接"}}</span>
        </li>
      </ul>
    </div>
    <!-- 精准开户 -->
    <div class="content" v-if="spanActive ==0">
      <!-- 开户类型 -->
      <div class="row" v-if=" personal_commission_mode!='mode_b'">
        <label>开户类型</label>
        <div :class="{open_type:open_type ==1}" @click="tochangetype(1)">代理</div>
        <div :class="{open_type:open_type ==0}" @click="tochangetype(0)" v-show="personal_commission_mode!='mode_a'">会员</div>
      ...
      <!-- 彩票返点 -->
      <div class="caipiaoContent" v-if="personal_commission_mode!='mode_b'">
        <h3>分润设置</h3>
        <div
          class="bar"
      ...
      <!-- 反水设置 -->
      <div class="refundContent" style="top:168px" v-if="personal_commission_mode!='mode_b'">
        <div
          class="bar"
      ...
      <div class="submitPay" @click="submitOpen" :class="{'submitPayPerson':personal_commission_mode=='mode_b'}">确认提交</div>
    </div>
    <!-- 生成邀请链接 -->
    ...
    <div class="content" v-if="spanActive ==2">
      ...
      <div class="row" v-if="personal_commission_mode !='mode_b'">
        ...
        <div :class="{open_type1:open_type1 ==2}" @click="changeUserType(2)" v-show="personal_commission_mode!='mode_a'">全部</div>
        <div :class="{open_type1:open_type1 ==1}" @click="changeUserType(1)" :style="{marginLeft:marginLeft+'px'}">代理</div>
        <div :class="{open_type1:open_type1 ==0}" @click="changeUserType(0)" v-show="personal_commission_mode!='mode_a'">会员</div>
      </div>
      <div class="row" v-if="personal_commission_mode =='mode_b'">
         <div class="newcode" @click="domainAdd">{{site_model=="invite_code"?"创建邀请码":"创建邀请链接"}}</div>
      </div>
      <!-- 表格 -->
    ...
    <div class="point_ball" v-if="pointBall">
      <div class="point_table">
        ...
        <div class="point_table_middle">
          <dl>
            <dd>
              <Cascader :data="options" v-model="selectedOptions" @on-change="handleChange"></Cascader>
            </dd>
            <dd style="margin-left:26px;margin-right:10px">
          ...
          render: (h, params) => {
            return h("div", [
              h(
                "span",
                {
                  class:[{'nodetail':this.personal_commission_mode =='mode_b'}],
                  style: {
                    marginRight: "4px",
                    cursor: "pointer",
              ...
              h("span", {
                ...
                style: {
                  display: this.personal_commission_mode =='mode_b'? "none" : "inline-block",
                  width: "1px",
                  height: "16px",
  ...
  methods: {
    ...
    set_refund2(list) {
      ...
      rerundList.forEach((v, e) => {
        if (v.key == "lottery") {
           ...
           list.forEach(v => {
              ...
              v.list.forEach((a, i) => {
               ...
               if(this.personal_commission_mode=="mode_a"){
                  a.option = a.option.sort(this.compare("value")).splice(0,4)
                }else{
          ...
          if(['qxcp','csj','fczj','tccp','tycp','flcp'].includes(this.$websiteName)){
    ...
    getPlayway() {
      ...
      this.$postS("agency/getPlaywaySet", {
        ...
        if(this.big=="1~5球"||this.big=="1~8球"||this.big=="1~3球"){
          this.big="第一球"
        }
        if (res && res.code == 200) {
