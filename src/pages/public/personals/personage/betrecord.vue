	...
	export default {
		...
		methods: {
			...
			getGameRecord() {
				...
				this.$getS(`member/bet-record/summary`, param).then(
					res => {
						if (res && res.code == 200) {
		...
		created() {
			this.$nextTick(() => {
				...
				if(['qxcp','csj','fczj','tycp','flcp'].includes(this.$websiteName)){  
						gameList = gameList.filter(item => item.name =='彩票游戏' || item.name=='棋牌游戏');
				}
				if(['935qp','632qp','kyqp','k78qp'].includes(this.$websiteName)){  
						gameList = gameList.filter(item => item.name =='棋牌游戏' || item.name=='捕鱼游戏'||item.name=='电子游艺'||item.name=='真人视讯');
				}
				gameList.forEach((item)=>{
					  item.list=item.list.filter(listItem=>listItem.status=='yes')
				})
