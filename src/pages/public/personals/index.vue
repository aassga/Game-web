    ...
    <div v-if="ifPersonal">
        <div class="peronsals">
            <div class="peronsals-content">
                <!-- 导航栏 -->
                <peronsal-aside class="peronsal-aside" ref="peronsalAside"></peronsal-aside>
                <!-- 关闭按钮 -->
                ...
                <div class="loading" v-if="loadingShow" ref="loading">
                    ...
                    <svg xmlns="http://www.w3.org/2000/svg" class="waves" width="1366" height="312" viewBox="0 24 150 28"
                        ...
                        <g>
                            <use class="speed-1" href="#wave-taHGs" x="80" y="-2" fill="url(#b1-taHGs)" transform="scale(1.5,1)"></use>
                            <use class="speed-2" href="#wave-taHGs" x="-10" y="4" fill="url(#b2-taHGs)" transform="scale(1.5,1)"></use>
                            <use class="speed-3" href="#wave-taHGs" x="70" y="8" fill="url(#b3-taHGs)" transform="scale(1.5,0.5)"></use>
                            <use class="speed-4" href="#wave-taHGs" x="80" y="-2" fill="url(#b3-taHGs)" transform="scale(1.5,0.5)"></use>
                        </g>
                    </svg>
                </div>
            </div>
        ...
        <!-- 打开个人中心的阴影 -->
        <div class="zhie"></div>
        <payModal @refreshList="onRefresList"></payModal>
        <rechargeLog></rechargeLog>
        <paymentModal @refreshModal="onRefresModal"></paymentModal>
    </div>
</template>

<script>
import rechargeLog from './recharge/rechargeLog'
...
import recharge2 from './recharge/manual';
import recharge3 from './recharge/usdtTransfer';
...
// 借呗
import borrowMoney0 from './JieBei/freeMoney';
import borrowMoney1 from './JieBei/borrowMoney';
import borrowMoney2 from './JieBei/repayMoney';
import borrowMoney3 from './JieBei/borrowQuota';
import borrowMoney4 from './JieBei/borrowRecord';
...
import withdraw0 from './withdraw/request';
import withdraw1 from './withdraw/usdtRequest';
import withdraw2 from './withdraw/binding';
import withdraw3 from './withdraw/bindUsdt';
import withdraw4 from './withdraw/record';
...
import payModal from '../home/payActivityModal';
import paymentModal from './personal-modal/payModal.vue';
export default {
    data() {
        return {
            comPName: 'recharge0'
    ...
    methods: {
        onRefresList(list){
            ...
            bankList.some((item,index) => {
                if(item.id == id){
                    this.$refs.peronsalAside.$refs.navAll.toggle2(index,item)
                    return true
                } 
            }); 
    ...
    watch: {
        navView: {
            handler: function(val, oldVal) {
                if (this.contentView == 'recharge') {
                    ...
                    if(this.itemDatas.classType=='bank')  this.comPName = this.contentView + 0
                    else if(this.itemDatas.classType =='paymentServiceLink') this.comPName = this.contentView + 2
                    else if(this.itemDatas.classType=='virtual') this.comPName = this.contentView + 3
                    ...
                    if(this.contentView == 'withdraw'){
                        if(this.$store.state.home.usdtWithdrawalsRate&&this.$store.state.home.usdtWithdrawalsRate!=='0'){
                            this.comPName = this.$store.state.personal.contentView + this.$store.state.personal.navView;
                             ...
                             let navView=0
                            if(this.$store.state.personal.navView==1) navView=2
                            else if(this.$store.state.personal.navView==2) navView= 4
                            else navView=0
                            this.comPName = this.$store.state.personal.contentView + navView 
                        }
                    }else{
                        this.comPName = this.$store.state.personal.contentView + this.$store.state.personal.navView;
                    }
        ...
        contentView: {
            handler: function(val, oldVal) {
                if(this.contentView == 'recharge'){  // 修改3 
                    if(this.itemDatas.classType=='bank')  this.comPName = this.contentView + 0
                    else if(this.itemDatas.classType =='paymentServiceLink') this.comPName = this.contentView + 2
                    else if(this.itemDatas.classType=='virtual') this.comPName = this.contentView + 3
                    else this.comPName = this.contentView + 1
        ...
        itemDatas:{
            handler: function(val, oldVal) {  // 修改4 
                if(this.contentView=='recharge'){
                    if(this.itemDatas.classType=='bank')  this.comPName = this.contentView + 0
                    else if(this.itemDatas.classType =='paymentServiceLink') this.comPName = this.contentView + 2
                    else if(this.itemDatas.classType=='virtual') this.comPName = this.contentView + 3
...
.peronsals {
    ...
    .peronsals-content {
        ...
        .peronsal-aside {
            ...
            background: url('/static/public/image/userImg/mockup_bg.png') right bottom no-repeat,
                url('/static/public/image/userImg/mackup_bg_right.png');
        }
        .vp-admin-wrap-close {
            position: absolute;
            z-index: 2000;
