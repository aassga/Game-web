  ...
  <div class="internerBank">
    ...
    <div class="content">
      <div class="bar">
        ...
        <Select v-model="depositId" @on-change="selectVal" class="bank1">
      ...
      <div class="bar" v-if="bankShow">
        ...
        <div class="bank" :style="{backgroundImage: 'url(' + depositDetail.imgUrl + ')', backgroundSize:'cover'}">
          ...
          <div class="row">
            <label>开户行：</label>
            <span class="cardNameOv">{{depositDetail.cardAddress}}</span>
      ...
      <div class="bar">
        ...
        <RadioGroup v-model="amount" class="RadioGroup" :class="[amountRadio.length>=8?'RadioGroupAmount':'']">
          <Radio :label="amountItem" v-for="(amountItem,i) in amountRadio" :key="i">
      ...
      <div class="bar">
        ...
        <Select v-model="type" class="bank2">
    ...
    >
      ...
      <div class="agent-con">
           ...
           <input type="text" placeholder="真实姓名" id="inputtext" v-model="realName" maxlength="15">
      </div>
  ...
  methods: {
    ...
    selectVal (val) {
      this.depositBank.forEach(v => {
        if (v.id == val){
           this.depositDetail = v 
           this.amountRadio = v.quick_amount.split(',')
        } 
      })
    },
    ...
    getbank () {
      ...
      this.$http.post(`${this.$HOST_NAME}/deposit/bank`).then(res => {
        if (res.code == 200) {
          ...
          this.amountRadio= res.data[0].quick_amount.split(',')
          console.log(this.amountRadio)
    ...
    sendmoney () {
      ...
      this.$http.post(`${this.$HOST_NAME}/deposit/application`, data).then(res => {
          if (res.code == 200) {
...
@keyframes _scaleLeave {
  ...
  /deep/ .ivu-modal {
    ...
    height: 318px;
    -webkit-animation:_scaleEnter .3s ease;
    animation:_scaleEnter .3s ease;
    &.ease-leave-active {
      -webkit-animation:_scaleLeave .3s ease;
