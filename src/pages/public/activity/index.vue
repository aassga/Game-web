    ...
    <div class="maincontainer" id="maincontainer" :style="styles" :class="classObj">
      <div class="main clear" :style="{width: widths+'px'}">
        <div class="content" :style="contentStyle">
          ...
          <div class="bannerHead" v-if="showNavLeft" :style="navStyle.bannerHead">
            <ul class="bannerUl" :style="navStyle.bannerUl">
              <li @click="fetchActive(item.id)" :class="{activeItem:activeId === item.id}" :style="navStyle.li"
                  v-for="(item,index) in headerList" :key="index">
                <div class="liItem">
          ...
          <!--  loading end-->
            <template v-if="showNavLeft">
              <div style="width: 960px;float: right; min-height: 500px; margin-bottom: -10px;">
      ...
      <!--弹窗-->
        <Modal
          ...
          <div style="text-align:center;overflow:hidden;">
            <Table class="showT" border :columns="columns" :data="data1" no-data-text="未查询到信息" disabled-hover v-if="showT"></Table>
            <p v-else>{{message}}</p>
          </div>
          <div slot="footer">
...
export default {
    ...
    return {
      ...
      columns:[
        ...
        },
        {
          ...
          render: (h, params) => {
            return h('div', [
              h('span',{},this.$moment(params.row.created_at*1000).format('YYYY-MM-DD HH:mm:ss'))
            ])
          }
        },
        {
          ...
          render: (h, params) => {
            return h('div', [
              h('span',{
                style:{
                  color:params.row.requirement_status=='yes'?'#59a8c9':params.row.requirement_status=='no'?'#777':'#fb374b'
                }
              },params.row.requirement_status=='yes'?'成功':params.row.requirement_status=='no'?'失败':'审核中')
            ])
          }
        }
      ],
  ...
  methods: {
      ...
      this.$nextTick(() => {
        ...
        for (let i = 0; i < floorLines.length; i++) {
         ...
         const _Elements = document.querySelectorAll(".applyBtn");
            const Imgs = `
              <img src="/static/public/image/apply-way.png" class="applyWay">
              <img src="/static/public/image/click-get.png" class="applyBtnImg">
            `;
            const checkImg = `
              <img src="/static/public/image/check-btn.png" class="checkBtn">
            `;
        ...
        downLinks.forEach(element => {
          element.href = '/static/' + this.$websiteName + '/html/download/index.html'
        })
      })
    },
    showModel(id){
    ...
    getHandle(id, element) {
      ...
      if (localStorage.token) {
        switch (id) {
          ...
          default:
            let that = this
            this.$http.get(`${this.$HOST_NAME}/activityApply`, { params: { class: id }}).then(function(res){
              if(res&&res.code==200){
                that.showT=false;
                that.message=res.data
              } else if (res && res.code == 5011) {
                that.$store.commit('alert/changbindPhone', true)
                return
              } else if (res && res.code == 5012) {
                that.$store.commit('alert/changbindbank', true)
                return
              } else if (res && res.code == 5013) {
                window.open('/static/' + this.$websiteName + '/html/download/index.html', '_blank')
                return
              } else {
                that.showT = false;
    ...
    getHandle1 (id, element) {
      ...
      this.showT = false
      this.title = '提示信息'
      this.message = '庆祝欧洲杯6月12日00:00正式开始申请'
      this.modal1 = true
      return */
      if (localStorage.token) {
        switch (id) {
    ...
    checkPlan(id, element) {
      ...
      if (localStorage.token) {
        let that =this
        this.$http.get(`${this.$HOST_NAME}/activityApplyListShow`, { params: {
          page: 1,
          class: id
        }}).then(function(res){
          if(res&&res.code==200){
              ...
              // that.data1=[
              //   {userName:'小强',created_at:'1605953908',requirement_status:'yes'},
              // ]
            }else{
              that.showT = false;
              that.message = res.message
    ...
    fetchActive (activeId) {
      ...
      this.$nextTick(() => {
        ...
        const applyImg = `<img src="/static/public/image/apply.gif" class="applyButtonImg1">`
        applyBtn.forEach(element => {
          element.innerHTML = applyImg
          // 排除充值和洗码
          if (element.attributes.id.value !== "A999" && element.attributes.id.value !== "A888") {
          }
        ...
        const applyImg1 = `<img src="/static/public/image/redbao.png" class="applyButtonImg2">`
...
/deep/.modal1 {
  ...
  .ivu-modal-header {
    ...
    color: #000000;
    padding: 0;
    background-image: linear-gradient(to bottom, #d2ad6d 0%, #f3d9ad 100%);
  }
  .ivu-modal-body {
    font-size: 20px;
    ...
    .ivu-modal {
      ...
      .ivu-modal-close {
        ...
        .vp-admin-wrap-close {
          ...
          &.amxpj1 {
            &:hover {
              background: linear-gradient(to right, rgb(96, 65, 228), rgb(65, 37, 140));
            }
          }
          &.cpxpj {
            &:hover {
