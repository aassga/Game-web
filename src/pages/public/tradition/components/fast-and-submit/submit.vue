<template>
  <div class="lottery-content-all-right-submit">
    ...
    <div class="decial-right-submit">
      <input @keyup="fastInput" v-model="saveSelectMoney.money" placeholder="输入自定义金额" type="text" :maxlength="maxlength">
  ...
  export default {
    ...
    methods: {
      ...
      chooseFastMoney (datas, index) {
        ...
        this.selectFast.list.forEach((item,v)=>{
             if(item.active){
                this.saveSelectMoney.money += (+item.price)
             }
        })
        // if (datas.active) {
      ...
      fastInput () {
        if (this.dInvalidMoneynew(this.saveSelectMoney.money)) {
          this.$emit('submit-Input-say')
        } else {
      ...
      review () {
        if(this.confirmbet){
          let total=this.totalCount.total
          this.totalCount.total=0
