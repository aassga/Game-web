    ...
    <div class="notice-bar-wrapper" :style="{
        ...
        <p v-for="(item, index) in noticeList" :key="'notice-bar-item-' + index" :style="{
          ...
          'transition-duration': `${itemDuration / rollSpeed}s`
            ...
            <span v-if="isHtml" style="font-size:18px;">恭喜玩家{{item.text.userName}}在【{{item.text.lotteryName}}】击中奖池<span class="item-bar-red">{{item.text.amount}}</span>元</span>
            <span v-else style="font-size:18px;" class="item-bar-span" :class="item.text.uid == $store.state.mainState.userinfo.uid ? 'item-bar-open' : ''">恭喜玩家{{item.text.userName}}在【{{item.text.lotteryName}}】击中奖池<span class="item-bar-red">{{item.text.amount}}</span>元</span>
        </p>
    </div>
</div>
</template>
...
export default {
    ...
    watch: {
        ...
        prizeOpen:function(val){
            if(this.prizeOpen) { 
                this.resetWrapMove()
                setTimeout(()=>{
                    this.$parent.prizeOpen=false
                },2000)
            }
        }
    },
