            ...
            <div :key="indexOut" v-for="(itemOut,indexOut) in listTop" class="decial-1-item">
              <div class="all-item">
                <div :key="indexInner" @click="clickFocus(itemInner)" v-for="(itemInner,indexInner) in itemOut.mess"
                     class="box">
                  <span :class="{'active':itemInner.active}" class="theBall">{{indexInner<4?itemInner.ball:itemInner.ball}}</span>
      ...
      async oddsBelong () {
        this.listTop.forEach((outItem, v) => {
          ...
          outItem.mess.forEach((inItem, v) => {
            let oddballs=inItem.ball.split("点")[0]
            inItem.code = '20501'
            inItem.price = this.oddsList['20501_' + oddballs + ''].odds
            inItem.fixedOdds = this.oddsList['20501_' + oddballs + ''].odds
            inItem.fsrate = this.oddsList['20501_' + oddballs + ''].fsrate
          })
        })
      },
      // 提交前检查，展示
