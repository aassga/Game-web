    ...
    <form ref="formDynamic">
      <div class="lottery-content-all-left">
        ...
        <div class="lottery-content-min-height">
          ...
          <div class="decial-1 alignItems sanTong">
            <div :key="indexOut" v-for="(itemOut,indexOut) in listDiff" class="decial-1-item">
              <div class="all-item">
                <div :key="indexInner" @click="clickFocus(itemInner)" v-for="(itemInner,indexInner) in itemOut.mess"
                     class="box">
                  <span :class="'bgActive' + itemThird.ball"
                        :key="indexThird" v-for="(itemThird,indexThird) in itemInner.list"
                        class="ksBall" v-if="itemInner.list.length"></span>
                  <span v-if="!itemInner.list">{{itemInner.ball}}</span>
                  ...
                  <input v-model="itemInner.model" @keyup="keyupInput(itemInner)" @keydown="keydownInput(itemInner)"
                         @click.stop="focusInput(itemInner)" :class="{'active':itemInner.active}" type="text" :maxlength="maxlength">
          ...
          <div class="decial-1 alignItems listQuick1">
            <div :key="indexOut" v-for="(itemOut,indexOut) in sanTong" class="decial-1-item">
              <div class="all-item">
                <div :key="indexInner" @click="clickFocus(itemInner)" v-for="(itemInner,indexInner) in itemOut.mess"
                     class="box">
                  <span :class="{'active':itemInner.active}" class="price">{{itemInner.ball}}</span>
                  <input v-model="itemInner.model" @keyup="keyupInput(itemInner)" @keydown="keydownInput(itemInner)"
...
<script>
  ...
  export default {
    ...
    data () {
      return {
        allData: data.ziXuan,
        listDiff: data.ziXuan[2].list,
        sanTong: data.ziXuan[3].list,
        confirmbet:true
      }
    },
    ...
    methods: {
      //分发赔率
      async oddsBelong () {
        this.listDiff.forEach((outItem, v) => {
          outItem.mess &&
          outItem.mess.forEach((inItem, v) => {
            inItem.code = '20503'
            inItem.price = this.oddsList['20503_' + inItem.ball + ''].odds
            inItem.fixedOdds = this.oddsList['20503_' + inItem.ball + ''].odds
            inItem.fsrate = this.oddsList['20503_' + inItem.ball + ''].fsrate
          })
        })
        this.sanTong.forEach((outItem, v) => {
          outItem.mess &&
          outItem.mess.forEach((inItem, v) => {
            inItem.code = '20503'
            inItem.price = this.oddsList['20503_' + inItem.ball + ''].odds
