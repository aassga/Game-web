      ...
      async oddsBelong () {
        ...
        this.listSum.forEach((outItem, v) => {
          outItem.mess && outItem.mess.forEach((inItem, v) => {
            let oddballs
            if (inItem.ball == '总和虎' || inItem.ball == '总和龙') {
              let code = '20604'
              //   if(this.childNeedMess.id=='6'){
              //     code='20904';
              //   }
              oddballs=inItem.ball.split("总和")[1]
              inItem.code = code
              inItem.price = this.oddsList[code + '_' +  oddballs + ''].odds
              inItem.fixedOdds = this.oddsList[code + '_' +  oddballs + ''].odds
              inItem.fsrate = this.oddsList[code + '_' +  oddballs + ''].fsrate
            } else {
              let code = '20603'
              //   if(this.childNeedMess.id=='6'){
              //     code='20903';
        ...
        this.listSum.forEach((outItem, v) => {
          outItem.mess && outItem.mess.forEach((inItem, v) => {
            if (inItem.active) {
              ...
              // reviewArr.push({
              //   code: '', title: inItem.title, ball: '总和' + inItem.ball,
