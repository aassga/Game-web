        ...
        <div class="lottery-content-min-height">
          ...
          <div class="decial-1">
            <div :key="indexOut" v-for="(itemOut,indexOut) in listTop" class="decial-1-item">
              <div class="all-item">
                <div @click="clickFocus(itemInner)" :key="indexInner" v-for="(itemInner,indexInner) in itemOut.mess"
                     class="box">
          ...
          <div class="decial-1 lotFive haveMargin">
            <div :key="indexOut" v-for="(itemOut,indexOut) in listBot" class="decial-1-item">
              ...
              <div class="all-item">
                <div :key="indexInner" @click="clickFocus(itemInner)" v-for="(itemInner,indexInner) in itemOut.mess"
                     class="box">
                  <span :class="{'active':itemInner.active}" class="theBall">{{indexOut==0?'特'+itemInner.ball:itemInner.ball}}</span>
                  <span :class="{'active':itemInner.active}" class="price">{{itemInner.price}}</span>
                  <input v-model="itemInner.model" @keyup="keyupInput(itemInner)" @keydown="keydownInput(itemInner)"
                         @click.stop="focusInput(itemInner)" :class="{'active':itemInner.active}" type="text" :maxlength="maxlength">
                  <vp-drop-down v-on:drop-down-say="seletPrice" v-on:drop-down-hide="hidePrice" :itemInner="itemInner"
  ...
  export default {
    ...
    data () {
      return {
        ...
        navigation: [{title: '特A'}, {title: '特B'}],
        allData: data.teMa,
        listTop: data.teMa[0].list,
        listBot: data.teMa[1].list,
    ...
    methods: {
      //分发赔率
      async oddsBelong () {
        let codeString = '20201'
        if (this.navSelect == '特A') {
          codeString = '20201'
        }
        if (this.navSelect == '特B') {
          codeString = '20202'
        }
