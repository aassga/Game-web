...
<script>
  ...
  export default {
    ...
    data () {
      return {
        allData: data.sanZiHeShu,
        list1: data.sanZiHeShu[0].list,
        list2: data.sanZiHeShu[1].list,
        confirmbet:true
      }
    },
    methods: {
      ...
      async oddsBelong () {
        ...
        this.list1.forEach((outItem, v) => {
          ...
          outItem.mess.forEach((inItem, v) => {
            inItem.code = '21014'
            inItem.price = this.oddsList['21014_' + inItem.ball + ''].odds
            inItem.fixedOdds = this.oddsList['21014_' + inItem.ball + ''].odds
            inItem.fsrate = this.oddsList['21014_' + inItem.ball + ''].fsrate
          })
        })
        this.list2.forEach((outItem, v) => {
          ...
          outItem.mess.forEach((inItem, v) => {
            inItem.code = '21015'
            inItem.price = this.oddsList['21015_' + inItem.ball + ''].odds
            inItem.fixedOdds = this.oddsList['21015_' + inItem.ball + ''].odds
            inItem.fsrate = this.oddsList['21015_' + inItem.ball + ''].fsrate
          })
        })
      },
      // 提交前检查，展示
