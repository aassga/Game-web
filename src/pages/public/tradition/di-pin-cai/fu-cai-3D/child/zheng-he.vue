          ...
          <div class="decial-1 lotFive">
            <div :key="indexOut" v-for="(itemOut,indexOut) in listBot" class="decial-1-item">
              <div class="all-item">
                <div :key="indexInner" @click="clickFocus(itemInner)" v-for="(itemInner,indexInner) in itemOut.mess"
                     class="box">
                  <span :class="{'active':itemInner.active}" class="theBall">{{itemInner.ball=='龙'||itemInner.ball=='虎'||itemInner.ball=='和'?itemInner.ball:'总和'+itemInner.ball}}</span>
                  <span :class="{'active':itemInner.active}" class="price">{{itemInner.price}}</span>
                  <input v-model="itemInner.model" @keyup="keyupInput(itemInner)" @keydown="keydownInput(itemInner)"
                         @click.stop="focusInput(itemInner)" :class="{'active':itemInner.active}" type="text">
                  <vp-drop-down v-on:drop-down-say="seletPrice" v-on:drop-down-hide="hidePrice" :itemInner="itemInner"
    ...
    methods: {
      ...
      async oddsBelong () {
        ...
        this.listCenter.forEach((outItem, v) => {
          ...
          outItem.mess.forEach((inItem, v) => {
            inItem.code = '21007'
            inItem.price = this.oddsList['21007_' + inItem.ball + ''].odds
            inItem.fixedOdds = this.oddsList['21007_' + inItem.ball + ''].odds
            inItem.fsrate = this.oddsList['21007_' + inItem.ball + ''].fsrate
          })
        })
        this.listBot.forEach((outItem, v) => {
          ...
          outItem.mess.forEach((inItem, v) => {
            if (
              ...
              inItem.code = '21002'
              inItem.price = this.oddsList['21002_' + inItem.ball + ''].odds
              inItem.fixedOdds = this.oddsList[
              '21002_' + inItem.ball + ''
                ].odds
              inItem.fsrate = this.oddsList['21002_' + inItem.ball + ''].fsrate
            } else {
              inItem.code = '21008'
              inItem.price = this.oddsList['21008_' + inItem.ball + ''].odds
              inItem.fixedOdds = this.oddsList[
              '21008_' + inItem.ball + ''
                ].odds
              inItem.fsrate = this.oddsList['21008_' + inItem.ball + ''].fsrate
            }
          })
        })
        this.listAdd.forEach((outItem, v) => {
          ...
          outItem.mess.forEach((inItem, v) => {
            inItem.code = '21009'
            inItem.price = this.oddsList['21009_' + inItem.ball + ''].odds
            inItem.fixedOdds = this.oddsList['21009_' + inItem.ball + ''].odds
            inItem.fsrate = this.oddsList['21009_' + inItem.ball + ''].fsrate
          })
        })
      },
      // 提交前检查，展示
