...
<script>
  ...
  export default {
    data () {
      return {
        isLoading: false,
        titleLeft: ['万位', '千位', '百位', '十位', '个位', '前三', '中三', '后三', '和值', '大小', '单双',"龙虎",'牛牛', '梭哈',],
        titleRight: ['万位', '千位', '百位', '十位', '个位'],
        resultBallLength: 5,
        ballLength: 10,
    ...
    methods: {
      ...
      async getHisDatas () {
        ...
        if (res && res.code == 200) {
          ...
          let rankConfig = [
            {index: 0, name: 'ww'}, {index: 1, name: 'qw'}, {index: 2, name: 'bw'}, {index: 3, name: 'sw'},
            {index: 4, name: 'gw'}, {index: 5, name: 'qs'}, {index: 6, name: 'zs'}, {index: 7, name: 'hs'}, {
              index: 8,
              name: 'hz'
            }, {index: 9, name: 'dx'},
            {index: 10, name: 'ds'},{index:11,name:"lh"}, {index: 12, name: 'nn'}, {index: 13, name: 'sh'},
          ]
