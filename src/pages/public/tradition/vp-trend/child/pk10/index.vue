            ...
            <div class="trend-left-box">
              <div class="trend-left-box-top">
                ...
                <div class="td-issue">
                 ...
                 <div class="td-issue" v-if="($route.path=='/trend/3630'||$route.path=='/trend/13'||$route.path=='/trend/12')&& newDatas.leftDatas.newHistoryIssue.length >0">
                  <div class="left-title">北京赛车期号</div>
                  <div :key="index" v-for="(item,index) in newDatas.leftDatas.newHistoryIssue" class="issue-number">
                    {{item}}
                  </div>
                </div>
                <!-- 开奖号 -->
              ...
              <div class="trend-center-canvas-list">
                <div class="vp-lottery-trend-cavans" :key="outInder" v-for="(outItem,outInder) in drawDeclList">
                  <div class="trend-cavans-title">
                    ...
                    <div class="t2">
                      <div :key="ballK" v-for="(ballItem,ballK) in ballLength">{{ballK+1}}</div>
                    </div>
                  </div>
...
<script>
  ...
  export default {
    data () {
      return {
        isLoading: false,
        titleLeft: ['冠亚大小', '冠亚单双', '冠亚和', '1龙虎', '2龙虎', '3龙虎', '4龙虎', '5龙虎'],
        titleRight: [
          '冠军',
          '亚军',
          '第三名',
    ...
    methods: {
      ...
      async getHisDatas () {
        ...
        if (res && res.code == 200) {
          ...
          this.newDatas.leftDatas.newHistoryIssue=[]
          ...
          let rankConfig = [ //xxx
            {index: 0, name: 'gydx'}, {index: 1, name: 'gyds'}, {index: 2, name: 'gyh'},
            {index: 3, name: 'lh1'}, {index: 4, name: 'lh2'}, {index: 5, name: 'lh3'},
            {index: 6, name: 'lh4'}, {index: 7, name: 'lh5'},
          ]
          ...
          // this.newDatas.total=res.data.data.gather.total;
          // this.newDatas.continuous =res.data.data.gather.continuous;
          // this.newDatas.forget=res.data.data.gather.forget;

          for (let i in res.data.data.gather.forget) {
            for (let j in res.data.data.gather.forget[i]) {
              this.newDatas.forget.push(res.data.data.gather.forget[i][j])
              
            }
          }

          for (let i in res.data.data.gather.total) {
            for (let j in res.data.data.gather.total[i]) {
              this.newDatas.total.push(res.data.data.gather.total[i][j])
            }
          }

          for (let i in res.data.data.gather.continuous) {
            for (let j in res.data.data.gather.continuous[i]) {
              this.newDatas.continuous.push(res.data.data.gather.continuous[i][j])
            }
          }
          ...
          res.data.data.list.forEach(item => {
            ...
            if(this.$route.path=='/trend/3630'|| this.$route.path=='/trend/13'|| this.$route.path=='/trend/12'){
                   let newitem;
                   if(item.historyIssue && item.historyOpenTime){
                     newitem=item.historyIssue+"期"+" "+"("+this.getYMD(item.historyOpenTime*1000)+")"
                   }else{
                     newitem="---"
                   }
                     this.newDatas.leftDatas.newHistoryIssue.push(newitem)
            }
            for (let i = 0; i < this.resultBallLength; i++) {
              let childArr = []
              for (let j = 1; j < 11; j++) {  //xxx
                childArr.push({ball: j, staue: false, lost: listunm[item.issue][i][j]})  //xxx
              }
