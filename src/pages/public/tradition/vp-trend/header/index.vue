<template>
  <div class="vp-trend-header">
    <nav class="vp-trend-nav">
      ...
      <div class="header-menu-right" :style="{right:headerRight}">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      // },
       trendSelectFc (item, index) {
        ...
        if (item.id == '999') {
          ...
          this.navDatasChi.list = item.games;
          this.navDatasChi.active = item.games[0].id
          // 向下传（默认第一个）
          this.childNeed = item.games[0]
          this.$store.commit('lottery/resetTrend', item.games[0])
          this.$router.push({
            path: `/trend/${this.childNeed.id}`
          })
        }
      ...
      //      console.log(this.getNavDatas)
      //    }
      // },
      getdatalist(){
           let saveDatas = [] 
           this.getNavDatas.list && this.getNavDatas.list.forEach((item) => { 
            if(item.games && item.games.length>1){
                 saveDatas.push(item.games[0])
            }
            item.games && item.games.forEach((innerItem) => {
              if (this.$route.path.split('/trend/')[1] == innerItem.id) {
                // 找到了向下传(父)
                this.childNeed = innerItem
                this.$store.commit('lottery/resetTrend', innerItem)
    ...
    // },
    getNavDatasFc(){
        let data = this.$getObjByLocalStorage(`gameSortV4_lottery`).list
        if(!data.length) return false
        data = data.filter(ele => ele.id != 219 )
        data.unshift({name: '全部彩票', games: [], id: '999'})
        this.getNavDatas.list = data
        this.getdatalist()
    }
    },
    watch: {
