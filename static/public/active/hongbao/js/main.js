...
$.ajax({
  ...
  success: function (res) {
    ...
    if (res && res.code === 200) {
      ...
      var tbody1 = '';
     if (arr1.length !== 0) {
       $('.section3 .box1 .content1').hide()
       $('.section3 .box1 .content2').show()
      ...
      if (arr2.length !== 0) {
        ...
        $('.section3 .box2 .content1').hide()
        $('.section3 .box2 .content2').show()
      ...
      * */
      if (Number(res.data.userData.depositAmount) >= 0) {
        if (Number(res.data.userData.depositAmount) < min) {
          ...
          $('.section2 #progress').css('width','25%')
        } else {
          /*进度提示*/
          $('.tishi1').hide()
          $('.tishi2').show()
          ...
          if (Number(res.data.userData.depositAmount) / max >= 0.25) {
            $('.section2 #progress').css('width',Number(res.data.userData.depositAmount) / max * 100 + '%')
          } else {
            $('.section2 #progress').css('width','25%')
          }
        /*title*/
          if (res.data.userData.giftMoney > 0) {
            $('.section2 .text').text('您已获得红包')
          }
        }
      }
    } else {
